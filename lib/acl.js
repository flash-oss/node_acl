...
Acl.prototype.addUserRoles = function (userId, roles, cb) {
    ...
    return this.backend.end(transaction).then((r) => (cb ? cb(null, r) : r), cb);
};
...
Acl.prototype.userRoles = function (userId, cb) {
    return this.backend.get(this.options.buckets.users, userId).then((r) => (cb ? cb(null, r) : r), cb);
};
...
Acl.prototype.roleUsers = function (roleName, cb) {
    return this.backend.get(this.options.buckets.roles, roleName).then((r) => (cb ? cb(null, r) : r), cb);
};
...
/**
  ...
  @return {Promise} Promise resolved when finished.
*/
Acl.prototype.removeRoleParents = async function (role, parents, cb) {
    contract(arguments)
...
Acl.prototype.removeResource = function (resource, cb) {
    ...
    return this.backend
        .get(this.options.buckets.meta, "roles")
        .then((roles) => {
            var transaction = this.backend.begin();
            ...
            return bluebird
                .all(
                    resources.map((resource) => {
                        var bucket = allowsBucket(resource);
                        return this.backend.get(bucket, role).then((permissions) => {
                ...
                .then(() => this.backend.end(transaction));
        })
        .then((r) => (cb ? cb(null, r) : r), cb);
};
...
Acl.prototype.isAllowed = function (userId, resource, permissions, cb) {
    ...
    return this.backend
        .get(this.options.buckets.users, userId)
        .then((roles) => {
            if (roles.length) {
...
Acl.prototype.areAnyRolesAllowed = function (roles, resource, permissions, cb) {
    ...
    if (roles.length === 0) {
        return bluebird.resolve(false).then((r) => (cb ? cb(null, r) : r), cb);
    } else {
        return this._checkPermissions(roles, resource, permissions).then((r) => (cb ? cb(null, r) : r), cb);
    }
};
